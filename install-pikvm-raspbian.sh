...
# Scripted Installer of Pi-KVM on Raspbian 32-bit and 64-bit RPi4, Zero 2W, and CM4 (CSI0)
#
...
get-packages() { 
  ...
  # Download each of the pertinent packages for rpi4/zero2w/zerow, webterm, and the main service
  for pkg in `egrep 'janus|kvmd' ${PKGINFO} | grep -v sig | cut -d'"' -f4 | egrep -v 'fan|oled' | egrep 'pi4|zero|janus|webterm|kvmd-[0-9]'`
  do
    rm -f ${KVMDCACHE}/$pkg*
...
get-platform() {
  # get pi model 
  model=$( tr -d '\0' < /proc/device-tree/model | cut -d' ' -f3,4,5 | sed -e 's/ //g' -e 's/Z/z/g' -e 's/Model//' -e 's/Rev//g' )
  ...
  case $model in 
    ...
    "3A") 
      ...
      platform="kvmd-platform-v2-hdmi-rpi3"
      export GPUMEM=96
      ;;
...
install-dependencies() {
  ...
  if [ ! -e /usr/bin/ttyd ]; then
    if [[ $( uname -m ) == "aarch64" || $( grep -i codename /etc/os-release | cut -d'=' -f2 ) == "bullseye" ]]; then
      ...
      #Install libwebsockets from source (v3.2 change) -- didn't work properly for 32-bit buster
...
#      make && make install
      ...
      ### 20220215 issue: ttyd won't compile on 32-bit buster so use the manual download of deb packages and install
...
### in place of reboot, we can try to create users and groups via method 2 here ###
